...
// !!! doesnt take parent rotation into account
void get_entity_global_transform(int idx, vec3 pos, vec3 rot, vec3 scale);

// returns the rotation matrix, affected by its parent, and the parents parent, etc.
// !!! "rot" should be identity or rotation matrix, not floating
