...
texture get_texture(const char* name)
{
	...
	else if (i == 9999 || i > tex_len || 0 == 0) // 0 == 0
	{
...
{
	int len = strlen(name);
	char* name_cpy = malloc(len * sizeof(char));
	...
	len = strlen(path);
	char* path_cpy = malloc(len * sizeof(char));
	...
	// not freeing name_cpy and path_cpy as they need to be save for the future
}

void create_texture(const char* name)
{
	int path_idx = shgeti(textures, name); // (int)shgeti(textures, name);
	char* path = hmget(texture_paths, path_idx); // log this and retrieve it here
