...
void setup_game_class(gravity_vm* vm)
{
    ...
    gravity_class_bind(c, "console", NEW_CLOSURE_VALUE(game_console));
    gravity_class_bind(c, "error", NEW_CLOSURE_VALUE(game_error));
    gravity_class_bind(c, "add_mesh", NEW_CLOSURE_VALUE(game_add_mesh));
...
{
    // SKIPPED: check nargs (must be 3 because arg[0] is self)
    if (nargs != 1) { throw_error("[Game.get_total_secs()] Wrong amount of arguments, 0 arguments are needed."); return; }
...
{
    // SKIPPED: check nargs (must be 3 because arg[0] is self)
    if (nargs != 1) { throw_error("[Game.get_delta_t()] Wrong amount of arguments, 0 arguments are needed."); return; }
...
{
    // SKIPPED: check nargs (must be 3 because arg[0] is self)
    if (nargs != 1) { throw_error("[Game.quit()] Wrong amount of arguments, 0 arguments are needed."); return; }
...
{
    // SKIPPED: check nargs (must be 3 because arg[0] is self)
    if (nargs != 2) { throw_error("[Game.load_level(string)] Wrong amount of arguments, 1 arguments are needed."); return; }

    gravity_value_t v1 = GET_VALUE(1);
    if (VALUE_ISA_STRING(v1) == BEE_FALSE)
    {
        throw_error("[Game.load_level(string)] Wrong argument type."); return;
    }
    ...
    {
        throw_error("[Game.load_level(string)] Scene does not exist."); return;
    }
...
#ifdef EDITOR_ACT
    // SKIPPED: check nargs (must be 3 because arg[0] is self)
    if (nargs != 2) { throw_error("[Game.console(string)] Wrong amount of arguments, 1 arguments are needed."); return; }

    gravity_value_t v1 = GET_VALUE(1);
    if (VALUE_ISA_STRING(v1) == BEE_FALSE)
    {
        throw_error("[Game.console(string)] Wrong argument type."); return;
    }

    // printf("passed ent name: %s\n", VALUE_AS_CSTRING(v1));
    char* msg = VALUE_AS_CSTRING(v1);
...
static bee_bool game_error(gravity_vm* vm, gravity_value_t* args, uint16_t nargs, uint32_t rindex)
{
    ...
    // printf("passed ent name: %s\n", VALUE_AS_CSTRING(v1));
    char* msg = VALUE_AS_CSTRING(v1);
    set_error_popup(GRAVITY_CALL_ERROR, msg);
    quit_game_act = BEE_TRUE;
#endif
}
static bee_bool game_add_mesh(gravity_vm* vm, gravity_value_t* args, uint16_t nargs, uint32_t rindex)
{
    if (nargs != 3) { throw_error("[Game.add_mesh(list<float>, string)] Wrong amount of arguments, 3 arguments are needed."); return; }

    gravity_value_t v1 = GET_VALUE(1);
    gravity_value_t v2 = GET_VALUE(2);
    if (VALUE_ISA_STRING(v2) == BEE_FALSE || VALUE_ISA_LIST(v1) == BEE_FALSE)
    {
        throw_error("[Game.add_mesh(list<float>, string)] Wrong argument type."); return;
    }

    char* name  = VALUE_AS_CSTRING(v2);
    gravity_list_t* g_verts = VALUE_AS_LIST(v1);

    // check list type
    if (g_verts->isa != gravity_class_float)
    {
        // throw_error("[Game.add_mesh(list<float>, string)] Wrong argument type. arg01 needs to be list<float>."); return;
    }

    // convert gravity list to f32 array
    gravity_value_t* verts = g_verts->array.p;
    f32* v = NULL;
    int  verts_len = g_verts->array.n;
    P_INT(verts_len);

    for (int i = 0; i < verts_len; ++i)
    {
        arrput(v, (f32)verts[i].f);
        P_F32(v[i]);
    }

    mesh m = make_mesh(v, verts_len, NULL, 0, name, BEE_TRUE);
    rtn_code rtn = add_mesh(m);
    // assert(0);
    arrfree(v);

    RETURN_VALUE(VALUE_FROM_INT(rtn), rindex);
}
