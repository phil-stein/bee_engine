...
void setup_ui_class(gravity_vm* vm)
{
    ...
    gravity_class_bind(c, "group_begin", NEW_CLOSURE_VALUE(gravity_group_begin));
    gravity_class_bind(c, "group_end", NEW_CLOSURE_VALUE(gravity_group_end));
    ...
    gravity_class_bind(c, "number", NEW_CLOSURE_VALUE(gravity_property_number));
...
{
    if (nargs != 3) { throw_error("[UI.group_begin(string, bool)] Wrong amount of arguments, 2 arguments are needed."); return; }
    gravity_value_t v1 = GET_VALUE(1);
    gravity_value_t v2 = GET_VALUE(2);
    if (VALUE_ISA_STRING(v1) == BEE_FALSE || VALUE_ISA_BOOL(v2) == BEE_FALSE)
    {
        throw_error("[UI.group_begin(string, bool)] Wrong argument types."); return;
    }

    char* title = VALUE_AS_CSTRING(v1);
    bee_bool scrollbar = VALUE_AS_BOOL(v1);

    bee_bool b = nk_group_begin(g_ctx, title, scrollbar ? NK_WINDOW_NO_SCROLLBAR : 0);
    RETURN_VALUE(VALUE_FROM_INT(b), rindex);
}

static bee_bool gravity_group_end(gravity_vm* vm, gravity_value_t* args, uint16_t nargs, uint32_t rindex)
{
    if (nargs != 1) { throw_error("[UI.group_end()] Wrong amount of arguments, 0 arguments are needed."); return; }
...
static bee_bool gravity_property_number(gravity_vm* vm, gravity_value_t* args, uint16_t nargs, uint32_t rindex)
{
    if (nargs != 3) { throw_error("[UI.number(string, number)] Wrong amount of arguments, 2 arguments are needed."); return; }
    gravity_value_t v1 = GET_VALUE(1);
    gravity_value_t v2 = GET_VALUE(2);
    if (!VALUE_ISA_STRING(v1) || !(VALUE_ISA_INT(v2) || VALUE_ISA_FLOAT(v2)))
    {
        throw_error("[UI.number(string, number)] Wrong argument types."); return;
    }
    ...
    if (VALUE_ISA_INT(v2))
    {
        int i = VALUE_AS_INT(v2);
        nk_property_int(g_ctx, str, INT32_MIN, &i, INT32_MAX, 1, 1);
        v = (f32)i;
    }
    else if (VALUE_ISA_FLOAT(v2))
    {
        v = VALUE_AS_FLOAT(v2);
        nk_property_float(g_ctx, str, -FLT_MAX, &v, FLT_MAX, 0.1f, 0.1f);
    }
    RETURN_VALUE(VALUE_FROM_FLOAT(v), rindex);
}
